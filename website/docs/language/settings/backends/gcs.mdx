...
The state is stored as an object under a configurable prefix in a [Google Cloud Storage](https://cloud.google.com/storage/) (GCS) bucket. The `bucket` argument is required and specifies the GCS bucket where the state file will be stored. If the bucket does not exist, Terraform will attempt to create it. This operation requires the `location` and `project` arguments to be set. For more information, see [Google Cloud Storage IAM roles](https://cloud.google.com/storage/docs/access-control/iam-roles).
...
## Creating the GCS bucket with Terraform
...
- `project` / `GOOGLE_BACKEND_PROJECT` - (Optional) The ID of the Google Cloud project where the GCS bucket should be created. Required only if you are using Terraform to create the GCS bucket.
- `location` / `GOOGLE_BACKEND_LOCATION` - (Optional) The location of the GCS bucket. Required only if you are using Terraform to create the GCS bucket. See [Bucket locations](https://cloud.google.com/storage/docs/locations) for valid values.
