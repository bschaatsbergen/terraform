...
- `project` - (Optional) The ID of the Google Cloud project where the GCS bucket should be created. Required only if you are using Terraform to create the GCS bucket.
- `location` - (Optional) The location of the GCS bucket. Required only if you are using Terraform to create the GCS bucket. See [Bucket locations](https://cloud.google.com/storage/docs/locations) for valid values.
